...
class TestManagerSync(unittest.TestCase):
    ...
    def test_two_tables_on_same_sqlite_path_share_backend(self):
        db_path = str(self.tmp_path / 'shared.db')
        pm = Manager()
        pm.register('users', Sqlite(path=db_path))
        pm.register('orders', Sqlite(path=db_path))

        # same path → same Backend instance via _BackendRegistry
        self.assertIs(pm._tables['users'], pm._tables['orders'])

        pm.create('users', 'u1', {'name': 'Alice'})
        pm.create('orders', 'o1', {'user': 'u1'})
        self.assertEqual(pm.read('users', 'u1'), {'name': 'Alice'})
        self.assertEqual(pm.read('orders', 'o1'), {'user': 'u1'})
    ...
    def test_reset_clears_singleton(self):
        pm1 = Manager()
        pm1.register('users', InMemory())
        self.assertTrue(pm1.exists('users'))

        Manager._reset()

        pm2 = Manager()
        self.assertFalse(pm2.exists('users'))
